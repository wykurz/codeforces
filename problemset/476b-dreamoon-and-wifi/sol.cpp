...
int main()
{
  ...
  REP(i, n) {
    mod *= 10;
    switch (s2[i]) {
    case '+': REP(j, L - 1) dp[j + 1][i + 1] = 10 * dp[j][i]; break;
    case '-': REP(j, L - 1) dp[j][i + 1] = 10 * dp[j + 1][i]; break;
    case '?': {
        REP(j, L - 1) dp[j + 1][i + 1]  = 5 * dp[j][i];
        REP(j, L - 1) dp[j][i + 1] += 5 * dp[j + 1][i];
        break;
    }
    default: throw std::runtime_error("inconceivable!");
    }
  }
  ll ans = dp[b + pfinal][n];
  cout << ans / mod << "." << setw(n) << setfill('0') << ans % mod << "\n";
}
